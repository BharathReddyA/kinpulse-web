...
const contentPages = {
  ...
  privacy: {
    ...
    sections: [
      ...
      ['Health and voice information', 'The current Site does not operate a Kithline device and does not collect check-in audio, transcripts, medication responses, mood signals, or device telemetry. Any future product that processes this information will require additional, product-specific notices, consent flows, retention controls, vendor review, and security documentation before launch.'],
  ...
  terms: {
    ...
    intro: 'These Terms of Use govern access to the Kithline concept website and founding-pilot application. By using the Site or submitting an application, you agree to these Terms. If you do not agree, do not use the Site.',
    sections: [
      ['Concept-stage service', 'Kithline is currently a product concept in development. Images, interfaces, features, pricing, timelines, technical descriptions, and pilot plans are illustrative and may change or may never become commercially available. Nothing on the Site is an offer to sell a finished product.'],
...
function App() {
  ...
  useEffect(() => {
    const pages: Record<string, { title: string; description: string }> = {
      ...
      '/privacy': {
        ...
        description: 'Learn how Kithline handles founding-pilot applications, website information, privacy choices, and future product data.',
      },
      '/terms': {
        title: 'Terms of Use | Kithline',
        description: 'Read the terms governing the Kithline concept website and founding-pilot application.',
      },
    }
  ...
  useEffect(() => {
    ...
    localStorage.setItem('kithline-theme', theme)
  ...
  const submitApplication = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('https://formsubmit.co/ajax/bharath.alluri@outlook.com', {
        ...
        body: JSON.stringify({ ...data, _subject: 'New Kithline founding pilot application', _template: 'table' }),
      })
  ...
  return <>
    ...
    <main id="main">
      <section className="hero" id="top">
        <img src="/images/kithline-morning-hero.webp" width="1536" height="1024" fetchPriority="high" alt="Kithline voice device beside a coffee cup on a sunlit kitchen counter" />
        <div className="hero-shade"/>
        <div className="hero-copy"><p className="overline">A voice check-in for families living apart</p><h1>A small check-in.<br/><em>A little more<br/>peace of mind.</em></h1><p>Kithline starts one gentle conversation each day, listens for what matters, and lets your family know how things are going.</p><div><button className="button" onClick={openForm}>Apply for the pilot <Icon name="arrow"/></button><a className="plain-link" href="#morning">See one morning ↓</a></div></div>
      ...
      <section className="morning section" id="morning">
        ...
        <div className="conversation">
          ...
          <div className="speech device-speech last"><small>KITHLINE</small><p>Glad to hear it. Anything you’d like your family to know?</p></div>
      ...
      <div className="modal" role="dialog" aria-modal="true" aria-labelledby="pilot-form-title" ref={dialogRef}>
        ...
        {formStatus === 'sent' ? <div className="form-success"><span><Icon name="check"/></span><p className="overline">APPLICATION RECEIVED</p><h2>Thank you for trusting us with your story.</h2><p>Your details have been sent to the Kithline team. We’ll be in touch before pilot selection begins.</p><button className="button light" onClick={() => setFormOpen(false)}>Close</button></div> : <>
          ...
          <form onSubmit={submitApplication}>
            ...
            <label className="consent"><input type="checkbox" name="contact_consent" value="Yes" required/><span>I agree that Kithline may email me about this pilot. I understand this form is processed by a third-party email delivery service and should not include medical records or urgent information.</span></label>
